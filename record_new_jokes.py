#!/usr/bin/env python3
"""
Tell Me Honey - NEW JOKES recorder
Records the 267 new jokes in Honey's cloned voice, matching your existing
library exactly (SPEED 0.78, the settings that fixed the "too loud & fast" batch).

HOW TO RUN (Mac):
  1) Put this file AND honey_jokes_2part_NEW.json in the SAME folder.
  2) Open Terminal, cd into that folder, e.g.:
        cd ~/Desktop/tellmehoney
  3) First time only:   pip3 install requests
  4) Run it:            python3 record_new_jokes.py
...
# ─── your voice + the PROVEN joke settings ───
...
LINES_FILE = "honey_jokes_2part_NEW.json"
...
def main():
    ...
    if not os.path.exists(LINES_FILE):
        print(f"\n⚠️  Can't find {LINES_FILE}. Put it in the SAME folder as this script.\n")
        sys.exit(1)
    ...
    for i, item in enumerate(lines, 1):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                made += 1; print(f"[{i}/{total}] ✓ {fid}")
            elif r.status_code == 401:
                print("\n✗ Key not valid (401). Check your API key and run again.\n"); sys.exit(1)
            elif r.status_code == 429:
                print(f"[{i}/{total}] limit — waiting 20s then retrying…"); time.sleep(20)
                ...
                if r2.status_code == 200:
                    ...
                    failed += 1; print(f"[{i}/{total}] ✗ {fid} ({r2.status_code}) — run again to retry")
            else:
                failed += 1; print(f"[{i}/{total}] ✗ {fid} ({r.status_code})")
        except Exception as e:
            failed += 1; print(f"[{i}/{total}] ✗ {fid} (error: {e}) — run again to retry")
        time.sleep(0.25)
    ...
    if failed:
        ...
        print(f"All {total} clips are in ./{OUTPUT_DIR}/  🍯  Move them in with honey_voice, then push.")
